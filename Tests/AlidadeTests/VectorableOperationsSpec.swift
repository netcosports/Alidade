...
@testable import AlidadeGeometry
@testable import AlidadeVectors
...
class VectorableOperationsSpec: XCTestCase {
  ...
  func testInterintegerOperations() {
    ...
    expect(point * CGFloat(2.int)).to(equal(CGPoint([6, -8])))
    expect(point * CGFloat(2)).to(equal(CGPoint([6, -8])))
  }
